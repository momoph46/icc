...
//assign oem_dataout = (state==final)? 8'd0:{out_data[6],out_data[5],out_data[4],out_data[3],out_data[2],out_data[1],out_data[0],so_data};
reg [7:0]oem_reg;
...
always@(posedge clk, posedge reset)begin
	if(reset)
		...
		if(msb_t) 
		oem_reg <= {out_data[5],out_data[4],out_data[3],out_data[2],out_data[1],out_data[0],data_buf[s],data_buf[s+1]};
		else 
		oem_reg <= {out_data[5],out_data[4],out_data[3],out_data[2],out_data[1],out_data[0],data_buf[s],data_buf[s-1]};
	
	end
		
	else if(state==final || nxt_state==final) begin
		oem_reg <= 8'd0;
	end

end
assign oem_dataout = oem_reg;
